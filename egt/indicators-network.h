...
template<typename network_t>
string canonical(const network_t&n)
...
class network_selection_indicator : public selection_indicator<network_t,RNG_t>
{
protected:
  // reusable state vector for fixation simulation
  vector<double>state;
public:
  network_selection_indicator(RNG_t _rng)
    : selection_indicator<network_t,RNG_t>(_rng) {}
  ...
  bool is_fixation_candidate(const network_t&n)
  { return ::is_fixation_candidate(n);
  }
